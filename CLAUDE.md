...
- `UsageMonitor` (`@MainActor @Observable`) — the single source of truth. Polls every 5 min + on popover open. **Each provider refreshes independently** (separate `Task` per provider) so a slow or Keychain-blocked provider never stalls the other. Feeds window transitions to `Notifier` and exposes `worstRemainingFraction` for the icon.
...
- `AppDelegate` + `main.swift` — `.accessory` AppKit app, `NSStatusItem` + transient `NSPopover` hosting `ContentView`. The menu-bar glyph (`MenuBarIcon`, a template ring) is kept in sync via `withObservationTracking`.
