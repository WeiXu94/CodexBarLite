...
**Claude is strictly read-only and never refreshes, rotates, or writes a token itself.** Rotating the `claude` CLI's single-use refresh token from here could leave the CLI holding a dead token, and the hard rule is **never break Claude Code**. When our cached/stored token is expired or 401s, `ClaudeClient` asks the CLI to refresh *its own* Keychain token, then re-reads the store. The CLI stays the sole owner of the credential — so we can never break it.

The refresh trigger is the subtle part: **`claude auth status` does *not* refresh the token** — it's a purely local read of the stored credential (returns far faster than a network round-trip; an early version used it and the token never refreshed on wake, so a depleted session stayed "Unauthorized" until the user ran `claude` by hand). The token only rotates when the CLI makes a real **authenticated request**, and the cheapest no-usage way to force one is an interactive `/status` slash command — which only exists inside an interactive session, so a real TTY is required. So `runInteractiveStatusViaPTY` drives `claude` through an `openpty` pseudo-terminal in an isolated working dir (`<App Support>/<bundleid>/ClaudeRefresh`, so no project `CLAUDE.md` is picked up), sends `/status` (a status command, **consumes no usage**), and tears the session down. A periodic Enter auto-accepts the one-time "trust this folder" prompt for that dir; the session is SIGKILLed by process group on a hard timeout so a stuck child can't hang a poll. This mirrors upstream's `ClaudeStatusProbe.touchOAuthAuthPath`. The nudge is cooldown-gated (long 5 min after a *confirmed* refresh, short 30 s otherwise so a failed attempt retries soon; forced past the cooldown on a 401) and verified by re-reading the claude Keychain blob via the `security` CLI — we only treat it as success once we see the token actually changed / is no longer expired. Best-effort throughout: if it doesn't refresh, the next poll retries and normal `claude` use keeps the Keychain fresh. (`claude` is resolved by probing `~/.local/bin`, `~/.claude/local`, Homebrew, `/usr/local` since GUI apps get a minimal PATH.)
