...
**Claude Keychain prompt avoidance.** The `claude` CLI owns the `Claude Code-credentials` Keychain item, so any other app reading it trips macOS's XARA "wants to access" dialog. To avoid prompting on every poll, `ClaudeClient` (1) caches the loaded credentials in memory for 30 min so routine polls never touch the Keychain, and (2) marks background reads no-UI (`LAContext.interactionNotAllowed` + `kSecUseAuthenticationUIFail`) so a read that *would* prompt fails silently instead. Only **user-initiated** refreshes — popover open and the manual refresh button — pass `interactive: true` and may show the prompt; the `interactive` flag threads `ContentView`/`AppDelegate` → `UsageMonitor.refresh` → `ClaudeClient.fetch`. Once the user clicks **Always Allow** once, the no-UI background reads succeed silently thereafter. (Ad-hoc re-signing still changes the app identity per rebuild, so the *first* read after each `make install` needs one Always Allow.)
