...
**Claude Keychain prompt avoidance.** The `claude` CLI owns the `Claude Code-credentials` Keychain item, so any other app reading it trips macOS's XARA "wants to access" dialog. `ClaudeClient` layers its credential reads so that claude-owned item is touched as rarely as possible: **(1)** an in-memory cache (30 min) for routine polls within a session; **(2)** a *persistent* Keychain item that **we own** (`<bundleid>.claude-oauth-cache`) holding a verbatim copy of the last credentials blob — because we created it, reading it back never prompts, and unlike the in-memory cache it survives app restarts, so a relaunch normally serves the token from here instead of re-reading the claude item (this is what stops the repeated prompts "after a few days"); **(3)** the credentials file; **(4)** finally the claude Keychain. Whenever (3)/(4) yield a token it's written back into our own item via `savePersistentCache`. All non-interactive reads are marked no-UI (`LAContext.interactionNotAllowed` + `kSecUseAuthenticationUIFail`) so a read that *would* prompt fails silently and falls through instead. Only **user-initiated** refreshes — popover open and the manual refresh button — pass `interactive: true` and may show the prompt; the `interactive` flag threads `ContentView`/`AppDelegate` → `UsageMonitor.refresh` → `ClaudeClient.fetch`. The persistent copy is strictly read-only — the CLI stays the sole owner that mints/rotates the real token, so caching here can never break Claude Code. (Ad-hoc re-signing changes the app identity per rebuild, so the *first* read after each `make install` may still need one Always Allow; that one re-seeds our own cache item.) This mirrors upstream's `KeychainCacheStore`, trimmed to a single item with no ACL/owner bookkeeping.
